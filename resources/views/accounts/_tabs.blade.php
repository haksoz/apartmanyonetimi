...
<div class="hidden lg:flex flex-wrap gap-2">
    @if ($showTransfer)
        <button type="button" onclick="document.getElementById('transfer-dues-modal').classList.remove('hidden')" class="shrink-0 rounded-xl bg-orange-500 px-3 py-2 text-sm font-semibold text-white hover:bg-orange-600">
            Borç Devret
        </button>
    @endif
    ...
    @foreach ($tabs as $tab)
        <a href="{{ $tab['url'] }}"
           class="rounded-xl px-3 py-2 text-sm font-semibold text-white {{ $tab['color'] }} {{ $active === $tab['key'] ? 'ring-2 ring-offset-2 ring-slate-950' : '' }}">
            {{ $tab['label'] }}
        </a>
    @endforeach
</div>
...
<details class="lg:hidden relative group">
    <summary class="cursor-pointer list-none rounded-xl bg-slate-950 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800 flex items-center justify-end gap-2 focus:outline-none">
    ...
    <div class="absolute right-0 top-full mt-2 w-56 rounded-2xl bg-white p-3 shadow-lg ring-1 ring-slate-100 flex flex-col gap-2 z-20">
        @if ($showTransfer)
            <button type="button" onclick="document.getElementById('transfer-dues-modal').classList.remove('hidden')" class="block w-full text-left rounded-xl bg-orange-500 px-3 py-2 text-sm font-semibold text-white hover:bg-orange-600">
                Borç Devret
            </button>
        @endif
        ...
        @foreach ($tabs as $tab)
            <a href="{{ $tab['url'] }}"
               class="block w-full text-left rounded-xl px-3 py-2 text-sm font-semibold text-white {{ $tab['color'] }} {{ $active === $tab['key'] ? 'ring-2 ring-offset-2 ring-slate-950' : '' }}">
                {{ $tab['label'] }}
            </a>
        @endforeach
    </div>
