...
@section('content')
                                ...
                                <td class="px-5 py-3.5 text-right whitespace-nowrap">
                                    @if(($t->transactionable_type ?? '') === \App\Models\Payment::class && $t->allocations->isNotEmpty())
                                        <button type="button" data-toggle-alloc="alloc-{{ $t->id }}"
                                            ...
                                            {{ $account->type === \App\Models\Account::TYPE_SUPPLIER ? 'Kapattığı Giderler' : 'Kapattığı Borçlar' }}
                                        </button>
                                    @endif
                                </td>
                            </tr>
    ...
    <script>
        document.addEventListener('click', function (e) {
            ...
            btn.textContent = open ? 'Gizle' : (isSupplier ? 'Kapattığı Giderler' : 'Kapattığı Borçlar');
        });
