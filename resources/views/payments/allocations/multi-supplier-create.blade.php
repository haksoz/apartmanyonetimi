...
@section('content')
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm mb-6">
        <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-500 mb-3">Seçili Ödemeler</h2>
    ...
    {{-- Tahsis Formu --}}
    <form method="POST" action="{{ route('accounts.payments.multi-supplier-allocate.store', $account) }}" class="rounded-2xl bg-white p-6 shadow-sm">
        @csrf
        ...
        <div id="alloc-summary" class="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-5 py-4 text-sm hidden">
            <div class="flex flex-wrap gap-x-8 gap-y-2">
                ...
                <div>
                    ...
                    <span id="sum-remaining" class="ml-1 font-bold text-slate-900">{{ number_format($totalBudget, 2, ',', '.') }} TL</span>
                </div>
            </div>
        </div>
        ...
        <script>
            (function(){
                ...
                document.addEventListener('input', function(e){
                    if (e.target.matches('input[name^="allocations"][name$="[amount]"]')) updateSummary();
                });

                document.addEventListener('click', function(e){
                    const btn = e.target.closest('[data-fill-selector]');
                    if (btn) {
                        const input = document.querySelector(btn.getAttribute('data-fill-selector'));
                        if (!input) return;
                        input.value = toFloat(input.getAttribute('data-remaining')).toFixed(2);
                        updateSummary();
                    }
                });
