<form method="POST" action="{{ $action }}" class="max-w-2xl rounded-2xl bg-white p-6 shadow-sm">
    ...
    <div class="space-y-5">
        ...
        @if ($category?->is_system)
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Kullanım Tipi</label>
            ...
            <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                <input type="checkbox" checked disabled class="rounded border-slate-300 opacity-60">
                <span class="opacity-60">Aktif kategori</span>
